...
The AI is given all the context it needs to work and write back to TPEN. If the AI can make HTTP requests, it coomuinicate results directly to TPEN Services. If it cannot, the prompt instructs the AI to emit a compact JSON payload that you can paste back into the split-screen tool which can access TPEN Services instead.

## Using it inside TPEN

The tool is designed for the transcription workspace as a split-screen tool.  Project leaders can register TPEN-Prompts as one of their project's tools on the Project Management page.  Once configured, contributors will see it alongside the other workspace tools.  When opened via the Transcription Interface, TPEN hands the tool a context-filled payload so prompt generation is instant.
